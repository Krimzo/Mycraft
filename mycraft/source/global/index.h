...
constexpr bool operator==(const ChunkIndex& first, const ChunkIndex& second)
{
...
constexpr ChunkIndex operator+(const ChunkIndex& first, const ChunkIndex& second)
{
	return { first.x + second.x, first.z + second.z };
}
...
struct ChunkPosition
{
	...
	static constexpr ChunkPosition from_flt3(const flt3& world)
	{
		return {
...
constexpr bool operator==(const ChunkPosition& first, const ChunkPosition& second)
{
	return first.x == second.x && first.z == second.z;
}
...
constexpr ChunkPosition operator+(const ChunkPosition& first, const ChunkPosition& second)
{
	return { first.x + second.x, first.z + second.z };
}
...
struct BlockIndex
{
	...
	static constexpr BlockIndex from_flt3(const flt3& vec)
	{
		return {
			(int) floor(vec.x),
			(int) floor(vec.y),
...
constexpr bool operator==(const BlockIndex& first, const BlockIndex& second)
{
...
constexpr BlockIndex operator+(const BlockIndex& first, const BlockIndex& second)
{
	return { first.x + second.x, first.y + second.y, first.z + second.z };
}
...
struct BlockPosition
{
	...
	static constexpr BlockPosition from_index(ChunkPosition chunk_pos, const BlockIndex& index)
	{
		return {
			chunk_pos.x + index.x,
			index.y,
	...
	static constexpr BlockPosition from_flt3(const flt3& vec)
	{
		return {
			(int) floor(vec.x),
			(int) floor(vec.y),
...
constexpr bool operator==(const BlockPosition& first, const BlockPosition& second)
{
	return first.x == second.x && first.y == second.y && first.z == second.z;
}
...
constexpr BlockPosition operator+(const BlockPosition& first, const BlockPosition& second)
{
	return { first.x + second.x, first.y + second.y, first.z + second.z };
}
