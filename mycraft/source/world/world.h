...
struct World
{
	...
	Block* get_world_block(const BlockPosition& block_pos);
	...
	std::optional<HitPayload> cast_ray(const ray& ray, float reach = 5.0f);
	void adjust_by_normal(HitPayload& payload) const;

	bool chunk_visible(const plane& plane, int i) const;
