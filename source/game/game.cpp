...
Environment::Environment(kl::GPU& gpu) : sun(gpu, 4096)
{
}
...
Game::Game(World& world) : world(world), environment(world.system.gpu)
{
    ...
    int counter = 0;
    for (auto value : {Block::GRASS, Block::DIRT, Block::STONE, Block::COBBLE, Block::WOOD, Block::PLANKS,
                       Block::COBWEB, Block::ROSE, Block::DANDELION})
    {
...
void Game::state_playing_update_mouse_input()
{
    ...
    player.inventory.selected_slot =
        abs(Inventory::HORIZONTAL_COUNT + player.inventory.selected_slot - window.mouse.scroll()) %
        Inventory::HORIZONTAL_COUNT;
...
void Game::state_playing_update_portals()
{
    ...
    for (auto& portal : portals)
    {
        ...
        std::optional<flt3> opet_inter = obj_ray.intersect_aabb(PORTAL_OBJ_AABB);
        if (!opet_inter)
            continue;
        if ((*opet_inter - obj_prev_cam_pos).length() > (obj_player_pos - obj_prev_cam_pos).length())
            continue;
        obj_ray.origin += obj_ray.direction() * ((*opet_inter - obj_ray.origin).length() + kl::sqrt(3.0f));
        obj_ray.set_direction(-obj_ray.direction());
        opet_inter = obj_ray.intersect_aabb(PORTAL_OBJ_AABB);
        if (!opet_inter)
            continue;
        *opet_inter -= obj_ray.direction() * MOVE_BIAS;
        ...
        player.camera.position = (friend_mat * flt4(*opet_inter, 1.0f)).xyz();
