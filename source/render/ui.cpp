...
UIMesh::UIMesh(kl::GPU& gpu) : gpu(gpu)
{
}
...
UI::UI(Renderer& renderer) : renderer(renderer), m_product(renderer), m_mesh(renderer.game.world.system.gpu)
{
    ...
    const std::initializer_list<dx::LayoutDescriptor> ui_input_layout = {
        ...
        {"KL_Color", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, D3D11_APPEND_ALIGNED_ELEMENT, D3D11_INPUT_PER_VERTEX_DATA,
         0},
...
void UI::draw()
{
    ...
    for (auto& info_variant : m_product.render_info())
    {
        ...
        {
            gpu.draw_raster_direct(text_info->text);
        }
    }
...
void UI::reload_toolbar()
{
    ...
    for (int i = 0; i < Inventory::HORIZONTAL_COUNT; i++)
    {
        ...
        UIRectangle item{item_position, item_size, {}, atlas_uv(block, flt2{0.0f}), atlas_uv(block, flt2{1.0f}), 1.0f};
...
void UI::reload_main_menu()
{
    ...
    {
        game.resume();
        return; // because mouse.set_position(window.frame_center()) inside game.resume() will cause unintended button
                // press
    }

    if (ui_button("V-SYNC", {0.0f, 0.2f}, {0.5f, 0.1f}, game.world.system.vsync ? ENABLED_COLOR : DISABLED_COLOR,
                  DARK_TEXT_COLOR))
        game.world.system.vsync = !game.world.system.vsync;

    const bool is_wireframe = renderer.is_wireframe();
    if (ui_button("WIREFRAME", {0.0f, 0.05f}, {0.5f, 0.1f}, is_wireframe ? ENABLED_COLOR : DISABLED_COLOR,
                  DARK_TEXT_COLOR))
        renderer.set_wireframe(!is_wireframe);

    if (ui_button("RAYTRACING", {0.0f, -0.1f}, {0.5f, 0.1f},
                  (game.render_mode == RenderMode::TRACING) ? ENABLED_COLOR : DISABLED_COLOR, DARK_TEXT_COLOR))
