...
UITextFormatHandler::UITextFormatHandler(kl::GPU& gpu) : gpu(gpu)
{
}
...
kl::TextFormat const& UITextFormatHandler::get(float font_size)
{
    ...
    if (it != m_formats.end())
        ...
        return m_formats
            .emplace(font_size,
                     gpu.create_text_format(L"Roboto", DWRITE_FONT_WEIGHT_NORMAL, DWRITE_FONT_STYLE_NORMAL, font_size))
            .first->second;
}

UIProduct::UIProduct(Renderer const& renderer)
    : renderer(renderer), m_text_format_handler(renderer.game.world.system.gpu)
{
}
...
void UIProduct::append_text(std::string_view const& text, flt4 const& color, float font_height, flt2 position,
                            flt2 rect, bool hor_center, bool ver_center)
{
    ...
    info.text.box_top_left = position;
    info.text.box_bottom_right = {position.x + rect.x, position.y - rect.y};
    info.text.h_align =
        hor_center ? kl::HAlign::DWRITE_TEXT_ALIGNMENT_CENTER : kl::HAlign::DWRITE_TEXT_ALIGNMENT_LEADING;
    info.text.v_align =
        ver_center ? kl::VAlign::DWRITE_PARAGRAPH_ALIGNMENT_CENTER : kl::VAlign::DWRITE_PARAGRAPH_ALIGNMENT_NEAR;
...
{
    if (!m_render_info.empty() && std::holds_alternative<UIPointRenderInfo>(m_render_info.back()) &&
        std::get<UIPointRenderInfo>(m_render_info.back()).topology == topology)
        return std::get<UIPointRenderInfo>(m_render_info.back());
    else
        return std::get<UIPointRenderInfo>(m_render_info.emplace_back(
            UIPointRenderInfo{.topology = topology, .offset = (UINT)m_points.size(), .count = 0}));
}
