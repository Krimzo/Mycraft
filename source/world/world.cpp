...
World::World(System& system, int render_distance) : system(system)
{
...
{
    return BlockPosition::from_flt3(first_chunk_pos().to_flt3()) +
           BlockPosition::from_index(ChunkPosition::from_index(chunk_ind), block_ind);
}
...
Block* World::get_world_block(BlockPosition const& block_pos)
{
    ...
    auto& chunk = get_chunk(chunk_ind);
...
{
    flt3 render_dist_blocks{float(m_render_distance * CHUNK_WIDTH)};
...
std::optional<HitPayload> World::cast_ray(ray const& ray, float reach)
{
    struct HitChunk
    {
    ...
    for (auto& hit_chunk : hit_chunks)
    {
        for (int i = 0; i < (int)std::size(hit_chunk.chunk.blocks); i++)
        {
            ...
            Block const& block = *hit_chunk.chunk.get_block(block_ind);
            ...
            if (std::optional<flt3> inters_opt = ray.intersect_aabb(box))
            {
                float hit_distance = (*inters_opt - ray.origin).length();
                if (hit_distance < min_hit_dist)
                {
                    ...
                    payload.normal = kl::normalize(*inters_opt - box.center);
                    result = payload;
                }
            }
        }
...
bool World::chunk_visible(plane const& plane, int i) const
{
    ChunkIndex chunk_ind = ChunkIndex::from_int(i, width_chunks());
    ChunkPosition chunk_pos = first_chunk_pos() + ChunkPosition::from_index(chunk_ind);
    flt3 extremes[2] = {
...
void World::regenerate_all()
{
    ...
    kl::async_for(0, chunk_count(), [&](int i) { generator.generate_chunk_cached(get_chunk_pos(i), get_chunk(i)); });
    kl::async_for(0, chunk_count(),
                  [&](int i) { get_chunk(i).upload(get_chunk_pos(i), system.gpu, get_block_test()); });
    upload_tracing();
}

void World::regenerate_with_swap(ChunkIndex index_delta)
{
